...
let default = () => {
  <main style={pageStyles}>
    <title> {React.string("Home Page")} </title>
    <h1 style={headingStyles}>
      {React.string("Congratulations")}
      <br />
      <span style={headingAccentStyles}> {React.string(`— you just made a Gatsby site! `)} </span>
      <span role="img" ariaLabel="Party popper emojis"> {React.string(`🎉🎉🎉`)} </span>
    </h1>
    <p style={paragraphStyles}>
      {React.string("Edit ")}
      <code style={codeStyles}> {React.string("src/pages/index.js")} </code>
      {React.string(" to see this page update in real-time. ")}
      <span role="img" ariaLabel="Sunglasses smiley emoji"> {React.string(`😎`)} </span>
    </p>
    <ul style={listStyles}>
        ...
        links->Belt.Array.map(link =>
          <li
            ...
            style={ReactDOM.Style.combine(
              listItemStyles,
              ReactDOM.Style.make(~color=link.color, ()),
            )}>
            <span>
              <a
                style={linkStyle}
                href={`${link.url}?utm_source=starter&utm_medium=start-page&utm_campaign=minimal-starter`}>
                {React.string(link.text)}
              </a>
              {if link.badge {
                <span style={badgeStyle} ariaLabel="New Badge"> {React.string("NEW!")} </span>
              } else {
                <> </>
              }}
              <p style={descriptionStyle}> {React.string(link.description)} </p>
            </span>
          </li>
        ),
      )}
